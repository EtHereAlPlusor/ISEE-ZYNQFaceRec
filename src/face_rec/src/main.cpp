    ...
    if (key0_value == 1) { // 开始应用
        if (last_key0 == 0) {
            ...
            if (fmodel.is_open() && fmaskModel.is_open()) { // 训练文件存在
                ...
                // cout << "MaskedModel: " << maskedModel.size() << endl;
                model.erase(0);
                maskedModel.erase(0);
                // for (auto iter = maskedModel.begin(); iter != maskedModel.end(); ++iter) {
            ...
            if (key1_value!=last_key1) { // 调整了SW1，即add person
                ...
                if (ifname.is_open()) { // 文件存在
                    while(!ifname.eof()){
                        ...
                        if (!faceName.empty()){
                            Mat face = imread("/mnt/trainedData/faces/"+faceName, 2|4);
                            if (face.empty()){
                                cout << endl;
...
void recognizeAndTrainUsingCam(CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, CascadeClassifier &eyeCascade3)
{
    ...
    while (true) { 
        ...
        if ((pResults ? *pResults : 0) == 1) { // 只找到一张人脸
            fRect << (pResults ? *pResults : 0) << endl;
            rectangle(displayedFrame, faceRect[0], CV_RGB(255, 255, 0), 2, CV_AA);
            
            fRect << faceRect[0].x << " " << faceRect[0].y << " " << faceRect[0].width << " " << faceRect[0].height << endl; // 人脸范围

            // Draw light-blue anti-aliased circles for the 2 eyes.
            Scalar eyeColor = CV_RGB(0,255,255);
            if (leftEye[0].x >= 0) {   // Check if the eye was detected
                rectangle(displayedFrame, Rect(faceRect[0].x + searchedLeftEye[0].x, faceRect[0].y + searchedLeftEye[0].y, searchedLeftEye[0].width, searchedLeftEye[0].height), CV_RGB(255, 255, 0), 2, CV_AA);
                circle(displayedFrame, Point(faceRect[0].x + leftEye[0].x, faceRect[0].y + leftEye[0].y), 6, eyeColor, 1, CV_AA);
                fRect << faceRect[0].x + searchedLeftEye[0].x << " " << faceRect[0].y + searchedLeftEye[0].y << " " << searchedLeftEye[0].width << " " << searchedLeftEye[0].height << endl; // 左眼的搜索范围
                fRect << faceRect[0].x + leftEye[0].x << " " << faceRect[0].y + leftEye[0].y << endl; // 左眼
            }
            else {
                fRect << "-1 -1 -1 -1" << endl; // 未找到左眼
                fRect << "-1 -1" << endl;
            }
            if (rightEye[0].x >= 0) {   // Check if the eye was detected
                rectangle(displayedFrame, Rect(faceRect[0].x + searchedRightEye[0].x, faceRect[0].y + searchedRightEye[0].y, searchedRightEye[0].width, searchedRightEye[0].height), CV_RGB(255, 255, 0), 2, CV_AA);
                circle(displayedFrame, Point(faceRect[0].x + rightEye[0].x, faceRect[0].y + rightEye[0].y), 6, eyeColor, 1, CV_AA);
                fRect << faceRect[0].x + searchedRightEye[0].x << " " << faceRect[0].y + searchedRightEye[0].y << " " << searchedRightEye[0].width << " " << searchedRightEye[0].height << endl; // 右眼的搜索范围
                fRect << faceRect[0].x + rightEye[0].x << " " << faceRect[0].y + rightEye[0].y << endl; // 右眼
            }
            else {
        ...
        if (m_mode == MODE_STARTUP || m_mode == MODE_DETECT) {
            ...
            if (have_trained == 0 && rec_out_cnt == 0) {
                ...
                if (gotFaceAndEyes && (pResults ? *pResults : 0) == 1) { // 只找到一张人脸
                    ...
                    if (masked[0] == 0) { // 没戴口罩    
                        ...
                        int identity = getSimilarity(preprocessedFace[0], model, &similarity);
                        string outputStr;

                        if (similarity < UNKNOWN_PERSON_THRESHOLD && identity != 0) {
                        ...
                        // Verify whether the reconstructed face looks like the preprocessed face, otherwise it is probably an unknown person.
                        double similarity = 100000000.0;
                        int identity = getSimilarity(preprocessedFace[0], maskedModel, &similarity);
                        string outputStr;

                        if (similarity < UNKNOWN_MASKEDPERSON_THRESHOLD && identity != 0) {
                            // Identify who the person is in the preprocessed face image.
