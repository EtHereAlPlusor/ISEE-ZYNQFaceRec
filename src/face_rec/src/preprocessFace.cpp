...
void detectBothEyes(const Mat &face, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, CascadeClassifier &eyeCascade3, Point &leftEye, Point &rightEye, vector<Rect> *searchedLeftEye, vector<Rect> *searchedRightEye)
{
    ...
    //     if (searchedLeftEye) // 若找到了眼睛，则返回找到的眼睛的位置（矩形）
    ...
    //     if (searchedRightEye) // 若找到了眼睛，则返回找到的眼睛的位置（矩形）
...
void equalizeLeftAndRightHalves(Mat &faceImg)
{
    ...
    bluechannel = channels[0];          // b通道的图像
    equalizeHist(bluechannel, bluechannel); // 均衡化
    greenchannel = channels[1];         // g通道的图像
    equalizeHist(greenchannel, greenchannel);
    redchannel = channels[2];           // r通道的图像
    equalizeHist(redchannel, redchannel);
    merge(channels, leftSide);             // 合并通道
...
vector<Mat> getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, CascadeClassifier &eyeCascade3, bool doLeftAndRightSeparately, vector<Rect> *storeFaceRect, vector<Point> *storeLeftEye, vector<Point> *storeRightEye, vector<Rect> *searchedLeftEye, vector<Rect> *searchedRightEye, int **pResults, vector<int> *masked, unsigned char *pBuffer)
{
    ...
    // Net net = readNet("/mnt/cascade/face_mask_detection.caffemodel", "/mnt/cascade/face_mask_detection.prototxt"); // 加载模型
    ...
    *pResults = facedetect_cnn(pBuffer, (unsigned char*)(srcImg.ptr(0)), srcImg.cols, srcImg.rows, (int)srcImg.step);
    for(int i = 0; i < (*pResults ? **pResults : 0); i++)
	{
        ...
        double area;
        double mask_rate;  // 口罩所占比例

        cvtColor(faceImg, HSVImg, CV_BGR2HSV);
        Scalar lower_hsv_blue = Scalar(90,14,46);         // 颜色范围低阈值
        Scalar upper_hsv_blue = Scalar(124,255,255);      // 颜色范围高阈值
        Scalar lower_hsv_white = Scalar(0,0,221);       // 颜色范围低阈值
        Scalar upper_hsv_white = Scalar(180,30,255);     // 颜色范围高阈值
        Scalar lower_hsv_black = Scalar(0,0,0);         // 颜色范围低阈值
        Scalar upper_hsv_black = Scalar(180,255,74);      // 颜色范围高阈值
        ...
        // Mat out = (net.forward(("loc_branch_concat","cls_branch_concat"))); // 装成行向量
        // cout << out.size() << endl;
        ...
        // resize(gray, warped, warped.size()); // 统一大小
        ...
        if (height > width) {
            resize(gray, gray, Size(160*width/height,160), 0, INTER_CUBIC); // 双立方插值
        }
        else {
            resize(gray, gray, Size(160,160*height/width), 0, INTER_CUBIC); // 双立方插值
        }
