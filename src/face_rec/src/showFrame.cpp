    ...
    while (true) { 
        ...
        for (int i=0; i<faceNum; i++){
            ...
            if (rectWidth > 0) { // 检测出了人脸
                ...
                if (rectX<0 || rectX>=640 || rectY<0 || rectY>=480 || (rectX+rectWidth)>=640 || (rectY+rectHeight)>=480){
                    outofrange = true;
                }
                ...
                if (pointX > 0) { // 检测出了左眼
                    ...
                    stringstream ssleftRec;
                    ssleftRec << slefteyeRect;
                    ssleftRec >> rectX; 
                    ssleftRec >> rectY; 
                    ssleftRec >> rectWidth;
                    ssleftRec >> rectHeight;
                    // rectangle(displayedFrame, Rect(rectX, rectY, rectWidth, rectHeight), CV_RGB(255, 255, 0), 2, CV_AA); // 画出左眼搜索范围
                }
                else {
                    cout << "left eye did not detected!" << endl;
                }
                ...
                if (pointX > 0) { // 检测出了右眼
                    ...
                    // circle(displayedFrame, Point(pointX, pointY), 6, eyeColor, 1, CV_AA);
                    stringstream ssrightRec;
                    ssrightRec << srighteyeRect;
                    ssrightRec >> rectX; 
                    ssrightRec >> rectY; 
                    ssrightRec >> rectWidth;
                    ssrightRec >> rectHeight;
                    // rectangle(displayedFrame, Rect(rectX, rectY, rectWidth, rectHeight), CV_RGB(255, 255, 0), 2, CV_AA); // 画出右眼搜索范围
                }
                else {
                    cout << "right eye did not detected!" << endl;
                }
            }
        }
