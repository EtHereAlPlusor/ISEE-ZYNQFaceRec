...
void sccb_sendword(unsigned short value)
{
	unsigned char  i=0;
...
{
    int clock_fd = open(CLOCK_VALUE_PATH, O_WRONLY);    // clock
    static const char values_str[] = "1"; 
    if (clock_fd<0){
        cerr << "Failed to open gpio value for writing!" << endl;
        exit(1);
    }

	// unsigned long Value=0x1; // 0000 0001: clock是低位

	// rd32_buf = Xil_In32(iic_base_addr);
	// Value = rd32_buf | Value;
	// Xil_Out32(iic_base_addr,Value );

    if (write(clock_fd, &values_str, 1) < 0) { 
...
{
    int clock_fd = open(CLOCK_VALUE_PATH, O_WRONLY);    // clock
    static const char values_str[] = "0"; 
    if (clock_fd<0){
        cerr << "Failed to open gpio value for writing!" << endl;
        exit(1);
    }

	// unsigned long Value=0xfffe;
...
void DATA_HIGH(void)
{
    int data_fd = open(DATA_VALUE_PATH, O_WRONLY); // data
    static const char values_str[] = "1"; 
    if (data_fd<0){
        cerr << "Failed to open gpio value for writing!" << endl;
        exit(1);
	...
	// unsigned long Value=0x02; // 0000 0010: data是高位

	// rd32_buf = Xil_In32(iic_base_addr);
	// Value = rd32_buf | Value;
	// Xil_Out32(iic_base_addr,Value );

    if (write(data_fd, &values_str, 1) < 0) { 
        cerr << "Failed to write value!" << endl; 
        exit(1);
    } 
