...
const float UNKNOWN_MASKEDPERSON_THRESHOLD = 0.95f;
...
void initDetectors(CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, CascadeClassifier &eyeCascade3, CascadeClassifier &maskCascade)
{
    ...
    // Load the Mask Detection cascade classifier xml file.
    try {
        maskCascade.load(maskCascadeFilename);
    } catch (cv::Exception &e) {}
    if ( maskCascade.empty() ) {
        cerr << "Could not load the Mask Detection cascade classifier [" << maskCascadeFilename << "]." << endl;
    }
    else 
        cout << "Loaded the the Mask Detection cascade classifier [" << maskCascadeFilename << "]." << endl;
}
...
void recognizeAndTrainUsingWebcam(CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, CascadeClassifier &eyeCascade3, CascadeClassifier &maskCascade)
{
    Ptr<BasicFaceRecognizer> model;
    Ptr<BasicFaceRecognizer> maskedModel;
    ...
    // 读取数据集中的所有图片
    ifstream fidentity;
    fidentity.open("/mnt/identity.txt");
    int count = 0; // 每个人五张照片

    model = loadModel("/mnt/traineddata/trainedmodel.yml"); // 加载训练结果
    maskedModel = loadModel("/mnt/traineddata/maskedtrainedmodel.yml"); // 戴口罩的结果
    // faceLabels = model->getLabelsByString("labels");

    // Run forever, until the user hits Escape to "break" out of this loop.
    while (!fidentity.eof()) {
        string frameName; // identity.txt中保存有照片的文件名，据此读取照片
        getline(fidentity, frameName);
        ...
        if (!frameName.empty()){
            ...
            int masked = 0;   // 0表示没有戴口罩，1表示戴了口罩

            Mat preprocessedFace = getPreprocessedFace(displayedFrame, faceWidth, faceCascade, eyeCascade1, eyeCascade2, eyeCascade3, maskCascade, preprocessLeftAndRightSeparately, &faceRect, &leftEye, &rightEye, &searchedLeftEye, &searchedRightEye, &masked);
            ...
            if (gotFaceAndEyes) {
                ...
                if (masked == 0) { // 没戴口罩
                    ...
                    // double t = (double)cvGetTickCount();
                    reconstructedFace = reconstructFace(maskedModel, preprocessedFace);
                    // if (m_debug)
                    //     if (reconstructedFace.data)
                    //         imshow("reconstructedFace", reconstructedFace);
                    ...
                    if (similarity < UNKNOWN_MASKEDPERSON_THRESHOLD) {
                        // Identify who the person is in the preprocessed face image.
                        identity = maskedModel->predict(preprocessedFace);
                        outputStr = toString(identity);
                        mysimilarity = similarity;
                    }
                    else {
                ...
                Point ptThreshold;
                if (masked == 0) {
                    ptThreshold = Point(ptTopLeft.x, ptBottomRight.y - (1.0 - UNKNOWN_PERSON_THRESHOLD) * faceHeight);
                }
                else {
                    ptThreshold = Point(ptTopLeft.x, ptBottomRight.y - (1.0 - UNKNOWN_MASKEDPERSON_THRESHOLD) * faceHeight);
                }
                ...
                double confidenceRatio = 1.0 - min(max(mysimilarity, 0.0), 1.0);
            ...
            if (mysimilarity < UNKNOWN_PERSON_THRESHOLD || mysimilarity < UNKNOWN_MASKEDPERSON_THRESHOLD) { // 看一下识别出来的帧画面
                imwrite("/nfs/displayedframe/facerec.bmp", displayedFrame);
            }
...
int main(int argc, char *argv[])
{
    ...
    initDetectors(faceCascade, eyeCascade1, eyeCascade2, eyeCascade3, maskCascade);
    ...
    recognizeAndTrainUsingWebcam(faceCascade, eyeCascade1, eyeCascade2, eyeCascade3, maskCascade);
