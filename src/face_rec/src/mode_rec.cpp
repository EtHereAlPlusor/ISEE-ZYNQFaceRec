...
void recognizeAndTrainUsingWebcam(CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    // while (true) { // 测试时只读取一张图片

        // Grab the next camera frame. Note that you can't modify camera frames.
        Mat cameraFrame;
        // videoCapture >> cameraFrame;
        cameraFrame = imread("/nfs/test.bmp");
        if( cameraFrame.empty() ) {
            cerr << "ERROR: Couldn't grab the camera frame." << endl;
            exit(1);
        }
        ...
        frameName << "/nfs/out.bmp";
        imwrite(frameName.str(), displayedFrame);
        ...
        //     // Quit the program!
        //     printf("User is quitting the program!");
        //     // break;
        // }
