...
Ptr<BasicFaceRecognizer> learnCollectedFaces(const vector<Mat> preprocessedFaces, const vector<int> faceLabels, const string facerecAlgorithm)
{
    ...
    else if (facerecAlgorithm == "FaceRecognizer.Fisherfaces"){
    ...
    model->save("/nfs/trainedData/trainedModel.yml");

    return model;
}
Ptr<BasicFaceRecognizer> loadModel()
{
    Ptr<BasicFaceRecognizer> model;
    ...
    try{
      model->load("/nfs/trainedData/trainedModel.yml");
